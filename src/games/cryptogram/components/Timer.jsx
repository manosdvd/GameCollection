...
export default function Timer({ solved, loading, initialSeconds = 0, onTimeUpdate }) {
    ...
    return (
        <div className="flex items-center gap-1.5 text-white/50 font-medium px-3 py-1 bg-white/5 rounded-full text-xs sm:text-sm border border-white/10">
            <TimerIcon size={14} />
            <span className="tabular-nums font-mono text-primary">{formatTime(seconds)}</span>
        </div>
    );
}
